 ...
 * ```
 *
 * The other side of the puzzle is `expand_subtree`, where we use the MacroSubstitution
 ...
 * the `MacroSubstitution` we should take. We push to the stack when we enter a
 * repetition.
 *
...
class MacroExpander(val project: Project) {
    ...
    private fun findMatchingPattern(
        ...
        val (macroCallBody, ranges) = project.createAdaptedRustPsiBuilder(call.macroBody
            ?: return null).lowerDocComments()
