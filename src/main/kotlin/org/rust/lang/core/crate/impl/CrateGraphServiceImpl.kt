...
private class CrateGraphBuilder(val project: Project) {
    ...
    private fun lowerPackageDependencies(pkg: ProjectPackage): LoweredPackageDependencies {
        return when (val dependencies = pkg.pkg.dependencies.classify()) {
            is SpitDependencies.Classified -> {
            ...
            is SpitDependencies.Unclassified -> {
    ...
    private sealed class SpitDependencies {
        ...
        data class Unclassified(val dependencies: Collection<CargoWorkspace.Dependency>) : SpitDependencies()
    }

    private fun Collection<CargoWorkspace.Dependency>.classify(): SpitDependencies {
        ...
        return if (unclassified.isNotEmpty()) {
            ...
            // are unclassified
            SpitDependencies.Unclassified(unclassified + normal)
        } else {
            SpitDependencies.Classified(normal, dev, build)
        }
    }
    ...
    private fun NonLibraryCrates.lowerNonLibraryCrates() {
        ...
        val (depsWithLib, flatDepsWithLib) = if (libCrate != null) {
            ...
            Pair(normalAndTestDeps + libDep, LinkedHashSet<Crate>(flatNormalAndNonCyclicDevDeps).apply { add(libCrate) })
        } else {
